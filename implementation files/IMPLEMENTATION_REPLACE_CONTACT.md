...
- [x] Add a `status` column (default `'new'`) to the `audits` table so the admin dashboard can track progress (New, Contacted, In Progress, Closed).
- [x] Completely drop the `submissions` table from the Supabase database.
- [x] Update `app/admin/page.tsx` to query `.from("audits")` instead of `submissions`.
- [x] Update the TypeScript types inside `app/admin/page.tsx` (`interface Submission` -> `interface Audit`) to expect `biggest_problem`, `traffic_source`, and `revenue_range`.
- [x] Adjust the UI of the expanded table row in `app/admin/page.tsx` to render these new rich fields in place of the old `message` block.
